...
function rangeSessionClause(range: DateRange | null, col = "session_id"): { sql: string; params: Record<string, number> } {
  ...
  const row = db
    .prepare(
      `SELECT MIN(last_activity) AS first,
         ...
         FROM sessions
        WHERE last_activity IS NOT NULL AND ${rg.sql}`
    )
    .get(rg.params) as { first: number | null; last: number | null; days: number };
...
export function getActivityByDay(db: Db, range: DateRange | null = null): DayActivity[] {
  ...
  const rows = db
    ...
    .all(rg.params) as DayActivity[];
  if (!range) _activity = { key, val: rows };
  return rows;
}
...
export function getToolUsage(db: Db, range: DateRange | null = null): ToolUsage[] {
  ...
  // parsing every row. Scope to in-range sessions when a range is set.
  ...
  if (!range) _tools = { key, val: result };
  return result;
}
